...
exports.seedCategories = async (req, res, next) => {
    try {
        const addedCount = await pageCategoryService.seedCategories();

        if (addedCount > 0) {
            res.status(200).json({
                status: 'success',
                message: `Đồng bộ thành công. Đã tạo ${addedCount} danh mục mẫu.`
            });
        } else {
            res.status(200).json({
                status: 'success',
                message: `Hệ thống đã có danh mục, không cần đồng bộ thêm.`
            });
        }
    } catch (error) {
        next(error);
